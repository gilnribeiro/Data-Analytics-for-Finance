...
let hedgePortReturns = 
    spy
    |> Array.map(fun s -> 
        let ret = s.Return*wSpy + riskFreeRate*wRf
        {Time = s.Time ; Return = ret})
(*
...
hedgePortMap.TryFind 2

let longShortPortReturns = 
    ibm 
    |> Array.choose(fun ibmOb -> 
    let timeHedge = hedgePortMap.TryFind ibmOb.Time
    match timeHedge with
    | Some t  -> 
        let longShort = ibmOb.Return - t.Return
        Some {Time = t.Time ; Return = longShort}
    | None -> None
    )
...
*)
let alpha = longShortPortReturns |> Array.averageBy(fun x -> x.Return)
(*
...
open FSharp.Stats
let io = 
    let std = longShortPortReturns |> stDevBy(fun x -> x.Return)
    alpha/std
