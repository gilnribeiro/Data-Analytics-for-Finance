...
type Record2 = 
    {X: int; Y: int}
let instance = {X = 4; Y = 2}
(**
9. Explain why this expression gives an error when you try to run it: 4 + 7.0 
*)
// Because we are trying to sum different data types, int with float
...
|> List.map(fun (group, x) -> if group then List.min x else List.max x)
(**
...
[1.0 .. 10.0] |> List.map(fun x -> x * 4.0)

[10.0 .. 20.0] |> List.groupBy(fun x -> x >= 15.0)
